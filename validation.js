...
function emptyCheckSubject() {
   ...
   if (sub.length==0) {
      document.getElementById("error-sub").innerHTML = "phone no be required";
      return false;
   }
      if(sub.length !==10){
         document.getElementById("error-sub").innerHTML = "phone no should be 10 digits";
      return false;
      }
      if(!sub.match(/^[0-9]{10}$/)){
         document.getElementById("error-sub").innerHTML = "phone no should be digit";
      return false;
      }
...
$("#submit-form").submit((e) => {
      ...
      e.preventDefault();
      $.ajax({
         url: "https://script.google.com/macros/s/AKfycbxitHwCEXelIk-p50ZLrdo0_90Smd6JKTkw5MWrFPYexWFg8VzX-N9Bc_NRVC6XRmmfAw/exec",
